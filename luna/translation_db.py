...
class TranslationDb:
    ...
    def parse_script_cmds(cls, script, strings_by_content_hash,
        ...
        raw_cmds = [
            cmd.strip() for cmd in script.decode('utf-8', errors='replace').split(';')
            if cmd.strip()
        ]
    ...
    def from_mrg(cls, allscr_path, script_text_path):
        ...
        for i in range(offset_count):
            ...
            strings_by_offset[i] = data.decode('utf-8', errors='replace')
        ...
        script_names = [
            script_nam_raw[i:i + 32].decode('utf-8', errors='ignore').replace('\0', '').strip()
        ...
        # Use multiprocessing with fallback for Windows / frozen apps
        try:
